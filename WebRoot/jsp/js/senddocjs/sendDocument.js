...
$(function() {
	...
	var url = 'DocumentDealAction_quereyAll.do?myselfDeptID=' + myselfDeptID;
	if (nodeId == "now") {
		url = 'DocumentDealAction_quereyAll.do?myselfDeptID=' + myselfDeptID
				+ "&fyear=" + nodeText;
	} else if (nodeId == "search") {
		url = 'DocumentDealAction_quereyAllSerach.do?myselfDeptID='
				+ myselfDeptID + "&fyear=" + nodeText;
	} else if (nodeId == "dept") {
		url = 'DocumentDealAction_quereyAllSerach.do?myselfDeptID='
				+ myselfDeptID + "&txt_file_dept=" + nodeText;
	}else if (nodeId == "21") {
		url = 'DocumentDealAction_quereyAll.do?myselfDeptID='
			+ myselfDeptID;
	}else if(nodeId == "22") {
		url = 'DocumentDealAction_quereyAllSerach.do?myselfDeptID='
			+ myselfDeptID;
	...
	if (row) {
		...
		window.open(url, "_blank",
				"toolbar=yes, menubar=yes, resizable=yes,location=yes,status=yes,height="
						+ window.screen.availHeight + ", width="
						+ window.screen.availWidth);
		/*var width1 = window.screen.availWidth*0.9;
		var height1 = window.screen.availHeight*0.9;
		...
		var value = window.showModalDialog(url + '?id=' + row.id, row.id + '_'
				+ row.fDocumentStatus, 'dialogHeight='+height1+';'
				+ 'dialogWidth='+width1+';' + 'dialogTop=' + iTop + ';dialogLeft='
...
//打印现行文件数据
function printme() {
	var url = getParamsURL("DocumentDealAction_printme.do?");
	window.open(url);
}
//导出现行文件数据
